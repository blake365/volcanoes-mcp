...
async function buildWFSQuery(layer: string, filters: string[], limit = 50): Promise<string> {
...
async function getVolcanoNumberByName(volcanoName: string): Promise<number | null> {
	try {
...
function cleanResponse(data: any, verbose = false): any {
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
	...
	try {
		if (request.params.name === "search-volcanoes") {
			...
			const filters: string[] = [`StartDateYear >= ${startYear}`];
			...
			const filters: string[] = [`LastEruption >= ${cutoffYear}`];
			...
			const args = request.params.arguments as any;
			const filters: string[] = [`ExplosivityIndexMax >= ${args.min_vei || 4}`];
